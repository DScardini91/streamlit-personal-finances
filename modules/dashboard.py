...
def create_dashboard():
    ...
    def create_goal_vs_spent_bar_chart():
        ...
        spent_values = [(category_totals.get(cat, 0) / total_spent) * 100 if total_spent != 0 else 0 for cat in categories]

        data = {"Categorias": categories, "Metas (%)": goal_values, "Gastos (%)": spent_values}
        df_bar = pd.DataFrame(data)

        fig = px.bar(df_bar, x="Categorias", y=["Metas (%)", "Gastos (%)"], barmode="group", title="Metas vs Gastos Percentuais",
                     color_discrete_map={"Metas (%)": "darkblue", "Gastos (%)": "lightblue"})
        return fig

    # Gráfico de Barras Verticais: P&L
    def create_pnl_bar_chart():
        monthly_totals = df.groupby(df["Data"].dt.to_period("M"))["Valor"].sum().reset_index()
        ...
        fig = px.bar(monthly_totals, x="Data", y="Valor", color="Color", title="P&L Mensal", color_discrete_map={"green": "green", "red": "red"})
        return fig

    # Gráfico de Linhas: Receitas e Gastos
    def create_revenue_expense_line_chart():
        monthly_data = df.groupby([df["Data"].dt.to_period("M"), "Tipo"])["Valor"].sum().unstack().reset_index()
        ...
        if "Gasto" not in monthly_data.columns:
            monthly_data["Gasto"] = 0

        fig = px.line(monthly_data, x="Data", y=["Receita", "Gasto"], title="Receitas e Gastos Mensais")
    ...
    def create_expense_pie_chart():
        category_totals = df[df["Tipo"] == "Gasto"].groupby("Categoria")["Valor"].sum().reset_index()
        fig = px.pie(category_totals, values="Valor", names="Categoria", title="Distribuição de Gastos por Categoria")
        return fig
